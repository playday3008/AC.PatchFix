...
namespace hooks {
    namespace {
        using Tag = games::ac::rogue::FPSUnlockHook;

        games::ac::rogue::FrameTiming *g_frame_timing = nullptr;
    ...
    void HookTraits<games::ac::rogue::FPSUnlockHook>::on_reload(const Config &cfg) {
        float target = cfg.target.get();
        log::get()->trace("FPSUnlockHook: on_reload target={}", target);
        apply_fps_patch(target);
    }

    auto HookTraits<games::ac::rogue::FPSUnlockHook>::install(const Addrs &addrs) -> bool {
        ...
        g_frame_timing = reinterpret_cast<games::ac::rogue::FrameTiming *>(ft_ptr);
        ...
        apply_fps_patch(games::ac::rogue::registry().config<Tag>().target.get());
