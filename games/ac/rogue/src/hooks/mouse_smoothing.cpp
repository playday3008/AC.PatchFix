...
#pragma clang diagnostic pop
        ...
        using Tag = games::ac::rogue::MouseSmoothingHook;
        ...
        games::ac::rogue::MouseAxisFilter g_axis_x;
        games::ac::rogue::MouseAxisFilter g_axis_y;

        auto factor_active(float factor) -> bool {
            return factor >= games::ac::rogue::k_mouse_min_factor && factor < 1.0F;
        }
        ...
        struct MouseSmoothingFunctor {
            ...
            [[maybe_unused]] static void operator()(mem::Registers &regs) {
                ...
                const float factor = games::ac::rogue::registry().config<Tag>().factor.get();
                ...
                if (!games::ac::rogue::registry().enabled<Tag>() || !factor_active(factor) ||
    ...
    void HookTraits<Tag>::on_reload(const Config &cfg) {
        ...
        if (factor < games::ac::rogue::k_mouse_min_factor) {
            log::get()->warn("MouseSmoothing: SmoothingFactor {} is below the {} floor, "
                             "compensation off",
                             factor,
                             games::ac::rogue::k_mouse_min_factor);
        } else if (factor >= 1.0F) {
