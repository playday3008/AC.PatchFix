...
namespace hooks {
    auto current_aspect() -> std::atomic<float> & {
        static std::atomic<float> instance {games::game_data<games::ac::Rogue>::k_default_aspect};
        return instance;
    }

    namespace {
        using Data = games::game_data<games::ac::Rogue>;
        using Tag  = games::ac::rogue::ViewportFittingHook;
...
#pragma clang diagnostic pop
        ...
        struct ViewportRatioLoad {
            ...
            [[maybe_unused]] static void operator()(mem::Registers &regs) {
                ...
                const float ar = games::ac::rogue::registry().config<Tag>().aspect_ratio.get();
                if (ar > 0.0F) {
                ...
                const auto *display =
                    reinterpret_cast<const games::ac::rogue::DisplaySettings *>(regs.rax);
        ...
        struct CoordTransformHook {
            ...
            [[maybe_unused]] static void operator()(mem::Registers &regs) {
                auto *a5 = reinterpret_cast<games::ac::rogue::Vec2f *>(regs.r10);
    ...
    void HookTraits<games::ac::rogue::ViewportFittingHook>::on_reload(const Config &cfg) {
    ...
    auto HookTraits<games::ac::rogue::ViewportFittingHook>::install(const Addrs &addrs) -> bool {
