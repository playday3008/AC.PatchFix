...
namespace hooks {
    using games::ac::rogue::MultiMonitor;

    namespace {
        using Data = games::game_data<games::ac::Rogue>;
        using Tag  = games::ac::rogue::DisplayDetectionHook;
...
#pragma clang diagnostic pop
        ...
        struct DisplayFlagHook {
            ...
            [[maybe_unused]] static void operator()(mem::Registers &regs) {
                const auto *display =
                    reinterpret_cast<const games::ac::rogue::DisplaySettings *>(regs.rbx);
                ...
                auto mode = games::ac::rogue::registry().config<Tag>().multi_monitor.get();
                switch (mode) {
    ...
    void HookTraits<games::ac::rogue::DisplayDetectionHook>::on_reload(const Config &cfg) {
    ...
    auto HookTraits<games::ac::rogue::DisplayDetectionHook>::install(const Addrs &addrs) -> bool {
