...
#include "games/ac/syndicate/structs.hpp"

namespace hooks {
    namespace {
        using Tag = games::ac::syndicate::AspectRatioFixHook;
...
#pragma clang diagnostic pop
        ...
        struct MirrorAspect {
            ...
            [[maybe_unused]] static void operator()(mem::Registers &regs) {
                if (!games::ac::syndicate::registry().enabled<Tag>()) {
                    return;
                }

                auto *state = reinterpret_cast<games::ac::syndicate::RendererAspectState *>(
                    regs.rbx + games::ac::syndicate::k_renderer_aspect_offset);

                float ratio = games::ac::syndicate::registry().config<Tag>().aspect_ratio.get();
                if (ratio <= 0.0F) {
