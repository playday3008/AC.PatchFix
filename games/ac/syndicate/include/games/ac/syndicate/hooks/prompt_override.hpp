#pragma once

#include <cstdint>

#include <array>
#include <optional>
#include <string>
#include <string_view>
#include <tuple>
#include <utility>

#include "core/hooks/registry/config_base.hpp"
#include "core/hooks/registry/dep_list.hpp"
#include "core/hooks/registry/hook_traits.hpp"
#include "core/hooks/registry/ini_field.hpp"
#include "core/hooks/registry/parsers.hpp"

#include "games/ac/syndicate/game_data.hpp"

namespace games::ac::syndicate {
    enum class PromptType : std::uint8_t {
        Xbox        = 2,
        PlayStation = 5,
    };

    struct PromptOverrideHook {};
} // namespace games::ac::syndicate

namespace hooks {
    template<>
    struct default_parser<games::ac::syndicate::PromptType> {
        [[maybe_unused]] static auto operator()(const std::string &s)
            -> games::ac::syndicate::PromptType {
            using PT              = games::ac::syndicate::PromptType;
            constexpr auto table  = std::to_array<std::pair<std::string_view, PT>>({
                {"Xbox", PT::Xbox},
                {"PlayStation", PT::PlayStation},
            });
            auto           result = detail::parse_enum(s, table, PT::PlayStation);
            if (result != PT::Xbox && result != PT::PlayStation) {
                return PT::PlayStation;
            }
            return result;
        }
    };

    template<>
    struct HookTraits<games::ac::syndicate::PromptOverrideHook> {
        using Addrs        = games::game_data<games::ac::Syndicate>::ResolvedAddresses;
        using PatternField = std::optional<std::uintptr_t> Addrs::*;

        static constexpr std::string_view name = "PromptOverride";

        using hard_deps = dep_list<>;
        using soft_deps = dep_list<>;

        static constexpr auto required_patterns = std::array<PatternField, 1> {
            &Addrs::get_active_device_type,
        };
        static constexpr auto optional_patterns = std::array<PatternField, 0> {};

        struct Config : config_base<Config> {
            ini_field<games::ac::syndicate::PromptType> type {
                "Input", "PromptType", games::ac::syndicate::PromptType::PlayStation};

            static constexpr std::size_t field_count = 1;
            static constexpr auto        field_ptrs  = std::tuple {&Config::type};
        };

        static auto install(const Addrs &addrs) -> bool;
    };
} // namespace hooks
