...
class log {
  public:
    struct detail {
        static consteval auto extract_logger_name(const char *filepath) -> std::string_view;
    };
...
consteval auto log::detail::extract_logger_name(const char *filepath) -> std::string_view {
    ...
    auto        dot      = path.rfind('.');
    std::size_t stem_end = (dot != std::string_view::npos && dot >= stem_begin) ? dot : path.size();
